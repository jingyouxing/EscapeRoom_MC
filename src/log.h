...
* 3 ERROR :   Error event that still allow the appliction to continue running
*			  Default: Display on screen and save in file(PATHLOG_ERMCSYS)
...
#define log_error(fmt, arg...) \
	{ log_file = __FILE__; log_lin = __LINE__; log_fun =__func__; log_lev = ERROR; write_log(PATHLOG_ERMCSYS,fmt, ##arg);}
